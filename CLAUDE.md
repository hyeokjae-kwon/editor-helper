...
A growing suite of tools for textbook editors, presented as sidebar menu items in one app. Two-package monorepo, no shared root `package.json` — `client/` and `server/` are run and installed independently.

- **색상 분석 (imgfind)** — upload a PDF *or* a standalone image file (JPG/PNG/TIFF), and it reports which images are *not* CMYK (Gray/RGB/Indexed/etc.), on which pages, with thumbnail previews where possible. Fully implemented.
- **맞춤법 분석 (spellFind)** — will check a target document's spelling/spacing against a reference style-guide document. So far only the file-intake UI exists (two file pickers styled as a "조회조건" bar — see `spellFind.jsx`); no analysis logic yet. The reference material this is meant to be built against was OCR'd from a scanned government PDF via `scripts/ocrReference.swift`, but that extracted text was dropped from the repo (copyright risk) — see git history around commit `4a4f081`/`273bdfc` if picking this back up.
...
Whenever you add or modify source code in this repo, write Korean comments explaining it — assume the reader is a college freshman with little to no programming background (this repo has been used as a learning example). Comment generously: explain not just *what* a line does but *why*, and spell out non-obvious language/library behavior (e.g. PDF internals, async/await, React state) rather than assuming prior knowledge. Match the style already in `pdfAnalyzer.js`, `imageAnalyzer.js`, and `client/src/jsx/app.jsx`.

## File naming

New files (any package) start with a lowercase letter and use camelCase — e.g. `app.jsx`, `imgfind.jsx`, `pdfAnalyzer.js`. No `PascalCase` filenames, even for React components.

Sidebar menu labels (`sidebar.jsx`'s `MENU_ITEMS[].label`) are always Korean, even when the underlying feature/file/`view` id is an English name like `imgfind` or `spellFind` — e.g. the `imgfind` menu shows "색상 분석", `spellFind` shows "맞춤법 분석".
...
- `src/` splits into two subdirectories: `src/jsx/` (all components, entry point included) and `src/css/` (all stylesheets) — see "File naming" above for the filename-casing rule. `index.html` points at `/src/jsx/main.jsx`; `main.jsx` imports `../css/index.css` and mounts `app.jsx`.
- No router. `src/jsx/app.jsx` is the shell: it renders four stacked regions — `topFrame.jsx` (top bar, brand), a `.body-row` containing `sidebar.jsx` (nav-only, no page content) and `<main className="content">`, then `bottomFrame.jsx` (footer). `app.jsx` owns a single `view` state (`'home'` | `'imgfind'` | `'spellFind'`) passed to `Sidebar` as `view`/`onNavigate`, and looks it up in a `pages` object to pick what renders inside `<main>`. Adding a screen means adding one entry to `sidebar.jsx`'s `MENU_ITEMS` (id + Korean `label` — see "File naming" above) and one entry to `app.jsx`'s `pages` object with the same id.
- `home.jsx` is intentionally trivial — just a "환영합니다." heading, no state/props. `imgfind.jsx` holds all of that feature's state (selected file, analysis result, loading/error, the currently zoomed preview image) and does the fetch to `/api/analyze` directly with `FormData` (field name `file`, `accept="application/pdf,image/jpeg,image/png,image/tiff"` on the `<input>`). A 초기화(reset) button clears file/result/error state and resets the native file input via a `ref` (needed since `<input type="file">` is uncontrolled). `spellFind.jsx` is a stub so far: two file pickers (기준자료/검사대상), each wrapped in a `.criteria-box` that just prints the picked filename — `.criteria-box--target` recolors the 검사대상 one red so it reads as distinct from the blue 기준자료 one. No upload/analyze call wired up yet.
- Layout is `.layout` (column: top-frame / body-row / bottom-frame) → `.body-row` (row: `.sidebar` + `<main className="content">`). Add future tools as more `.side-nav-item` entries in `sidebar.jsx` plus a new `view` value, no router needed yet. `#root` has no `max-width` — the layout intentionally fills the full viewport width; `.result`/`.preview-grid` size themselves accordingly (`.preview-grid` is `repeat(auto-fill, minmax(160px, 1fr))`, not a fixed column count).
