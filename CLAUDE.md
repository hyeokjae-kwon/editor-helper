...
- No router. `src/jsx/app.jsx` is the shell: it renders four stacked regions — `topFrame.jsx` (top bar, brand), a `.body-row` containing `sidebar.jsx` (nav-only, no page content) and `<main className="content">`, then `bottomFrame.jsx` (footer). `app.jsx` owns a single `view` state (`'home'` | `'imgfind'`) passed to `Sidebar` as `view`/`onNavigate`, and swaps which component renders inside `<main>` — `home.jsx` (landing screen) or `imgfind.jsx` (the color-model analyzer). `imgfind.jsx` holds all of that feature's state (selected file, analysis result, loading/error, the currently zoomed preview image) and does the fetch to `/api/analyze` directly with `FormData` (field name `file`, `accept="application/pdf,image/jpeg,image/png,image/tiff"` on the `<input>`). A 초기화(reset) button clears file/result/error state and resets the native file input via a `ref` (needed since `<input type="file">` is uncontrolled).
