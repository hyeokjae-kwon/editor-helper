// ============================================================================
// 이 파일은 프론트엔드(화면에 보이는 부분)의 전체를 담당하는 단 하나의 컴포넌트입니다.
// React를 처음 보는 사람을 위해 간단히 설명하면:
//   - "컴포넌트"는 화면의 한 부분(여기서는 페이지 전체)을 그려내는 자바스크립트 함수입니다.
...
function App() {
  ...
  const [file, setFile] = useState(null) // 사용자가 선택한 PDF 파일 (아직 선택 안 했으면 null)
  ...
  const handleAnalyze = async () => {
    ...
    // 'pdf' 라는 이름(필드명)으로 파일을 담는데, 이 이름은 서버(server/src/index.js)의
    // upload.single('pdf') 부분과 반드시 똑같아야 서로 짝이 맞습니다.
  ...
  return (
    <div className="page">
      ...
      <div className="upload-box">
        {/* type="file" : 파일 선택창을 여는 input. accept로 PDF만 고를 수 있게 제한합니다. */}
      ...
      {result && (
        <div className="result">
          ...
          {/* CMYK가 아닌 이미지가 하나도 없다면(전부 정상이라면) 표/미리보기 대신 안내 문구만 보여줍니다. */}
          {nonCmykModels.length === 0 ? (
            <p className="summary">모든 이미지가 CMYK 색상모델입니다.</p>
          ) : (
            // <>...</> 는 "Fragment"라고 부르며, 불필요한 <div>를 추가하지 않고
            // 여러 요소를 하나로 묶어서 반환하고 싶을 때 사용합니다.
            <>
